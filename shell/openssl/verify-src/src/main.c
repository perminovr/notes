...
int main(int argc, char **argv)
{
		...
		switch (c) {
			...
			case 's': {
				if (optarg) {
					size_t len = strlen(optarg);
					if (len) {
						sign_fname = calloc(strlen(optarg)+1, 1);
			...
			case 'p': {
				if (optarg) {
					size_t len = strlen(optarg);
					if (len) {
						pem_fname = calloc(strlen(optarg)+1, 1);
	...
	if (pem_fname) {
		...
		pubpem = calloc(filestat.st_size, 1);
...
static int verify(char *file_fname, char *sign_fname, unsigned char *pubpem)
{
	...
	if ( file_d < 0 ) {
		printf("Error: cannot open %s", file_fname);
		return 1;
	}
	...
	if ( stat(file_fname, &filestat) != 0 ) {
		close(file_d);
		printf("Error: cannot get stat of %s", file_fname);
		return 1;
	}

	/* map file to memory */
	file_size = filestat.st_size;
	file_buf = mmap(0, file_size, PROT_READ, MAP_SHARED, file_d, 0);
	if ( file_buf == MAP_FAILED ) {
		close(file_d);
		printf("Error: cannot map %s to memory", file_fname);
		return 1;
	}
	...
	if ( sign_d < 0 ) {
		printf("Error: cannot open %s", sign_fname);
		return 1;
	}
	...
	if ( stat(sign_fname, &filestat) != 0 ) {
		close(sign_d);
		printf("Error: cannot get stat of %s", sign_fname);
		return 1;
	}

	/* read sign */
	sign_buf = calloc(filestat.st_size, 1);
	if ( read(sign_d, sign_buf, filestat.st_size) <= 0 ) {
		free(sign_buf);
		close(sign_d);
		printf("Error: cannot read %s", sign_fname);
		return 1;
	}
